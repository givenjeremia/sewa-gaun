<div class="card mt-2">
    <div class="card-header">
      <h3 class="card-title">Pengambilan Gaun</h3>
    </div>
    <!-- /.card-header -->
    <div class="card-body">
          ...
          <tr class=" bg-light">
            ...
            <td>
              <select class="form-control" id="status_option" name="status_option" onchange="statusChange(this)" pembayaran="{{ $value->id }}" gaun="{{ $value->gaun[0]->id }}">
                @foreach ([ 0 => 'Belum Di Ambil', 1 => 'Sudah Di Ambil'] as $key => $Label)
                <option value="{{ $key }}" {{ $value->gaun[0]->pivot->pengambilan == $key ? 'selected' : '' }}>{{ $Label }}</option>
                @endforeach
              </select>
            </td>
